// =============================================================
// VetRx — verify_phase5c_pdf_visual.mjs
// Phase 5C Comprehensive Visual Alignment & Page Verification
// =============================================================
...
const WORKSPACE_OUT_DIR = path.resolve('artifacts/phase5c-pdf-validation');
...
const BRAIN_OUT_DIR = path.join(BRAIN_ARTIFACT_DIR, 'phase5c_artifacts');
...
function startStaticServer() {
  return new Promise((resolve) => {
    ...
    server.listen(PORT, () => {
      console.log(`Phase 5C Validation Server listening at ${TARGET_URL}`);
      resolve(server);
    });
  });
}
...
async function run() {
  ...
  const userDataDir = path.join(os.tmpdir(), `edge-test-p5c-${Date.now()}`);
  ...
  async function testPrescriptionScenario(scenarioId, fileBaseName, medCount, options = {}) {
    ...
    scenarioResults.push({
      ...
      fileBaseName,
      pdfPath: `artifacts/phase5c-pdf-validation/${fileBaseName}.pdf`,
  ...
  async function putInvoiceWithItems(docType, itemCount, options = {}) {
    ...
    const invoiceNumber = `${docType === 'Tax Invoice' ? 'INV' : 'RCP'}-PH5C-${invId}`;
    ...
    for (let i = 1; i <= itemCount; i++) {
      ...
      const item = {
        ...
        discountAmtPaisa: 0,
        prescriptionId: 1001,
        prescriptionNumber: 'RX-PH5C-0001',
  ...
  console.log('\n>>> STARTING PRESCRIPTION SCENARIOS (9 core + variants) <<<');
  // 1. One medicine
  ...
  // 9. Long multi-page prescription
  await testPrescriptionScenario('Rx-9', 'prescription-long-content-multipage', 10, {
    longDiagnosis: true,
  ...
  for (const dir of [WORKSPACE_OUT_DIR, BRAIN_OUT_DIR]) {
    fs.writeFileSync(path.join(dir, 'phase5c-validation-summary.json'), JSON.stringify(summary, null, 2));
  }

  console.log('\n======================================================');
  console.log(`PHASE 5C PDF VISUAL VALIDATION COMPLETE`);
...
run().catch((err) => {
  console.error('Fatal execution error during Phase 5C validation:', err);
  process.exit(1);
});

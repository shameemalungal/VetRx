// =============================================================
// VetRx — verify_phase5d_comprehensive.mjs
// Phase 5D: Comprehensive Visual Alignment, Regression & Page Verification
// =============================================================
...
const WORKSPACE_OUT_DIR = path.resolve('artifacts/phase5d-pdf-validation');
...
const BRAIN_OUT_DIR = path.join(BRAIN_ARTIFACT_DIR, 'phase5d_artifacts');
...
function startStaticServer() {
  return new Promise((resolve) => {
    ...
    server.listen(PORT, () => {
      console.log(`Phase 5D Validation Server listening at ${TARGET_URL}`);
      resolve(server);
    });
  });
}
...
async function main() {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'vetrx-p5d-'));
  ...
  // Update practitioner in DB to match reference
  ...
  async function generateAndRenderPdf(elementId) {
    return page.evaluate(async (targetId) => {
      ...
      return { numPages, base64Pdf, pages };
    }, elementId);
  }
  ...
  async function putPrescriptionWithItems(medCount, options = {}) {
    ...
    if (options.longQual) {
      await page.evaluate(async () => {
        ...
        await db.practitioners.update(1, {
          qualifications: 'BVSc & AH, MVSc (Veterinary Gynaecology & Obstetrics), PhD (Reprod. Biotech), PG Diploma in Small Animal Practice',
        });
      });
    } else {
      await page.evaluate(async () => {
    ...
    for (let i = 0; i < medCount; i++) {
      ...
      const itemId = rxId * 100 + i + 1;

      let brandName = `${tmpl.b} #${i + 1}`;
      let genericName = tmpl.g;
      let sig = 'Administer as directed after feeding. Complete full course.';
      let route = tmpl.r;
      let frequency = tmpl.f;

      if (options.longName && i === 0) {
        brandName = 'Sulphadimidine Sodium Long Acting Concentrated Veterinary Suspension (Ultra Fortified 33.3% w/v)';
      }
      if (options.longGeneric && i === 0) {
        genericName = 'Sulphadimidine Sodium IP equivalent to anhydrous Sulphadimidine with Sodium Methyl Hydroxybenzoate 0.15% w/v';
      }
      if (options.longSig && i === 0) {
        sig = 'Administer orally mixed with crushed jaggery or treacle immediately following grain feeding. Ensure continuous access to clean fresh drinking water. Complete full course without premature cessation.';
      }
      if (options.longRoute && i === 0) {
        route = 'Deep Intramuscular in Upper Neck Cervical Muscle';
      }
      if (options.longFreq && i === 0) {
        frequency = 'Three times daily at 8-hour intervals (06:00, 14:00, 22:00)';
      }
      ...
      await putInDb('prescriptionItems', rxItem);
    }
  ...
  async function putInvoiceWithItems(docType, itemCount, options = {}) {
    const invId = 3000 + itemCount + Math.floor(Math.random() * 6000);
    const prefix = docType === 'Payment Receipt' ? 'RCP-PH5D' : 'INV-PH5D';
    const invoiceNumber = `${prefix}-${invId}`;
    const activePid = await page.evaluate(() => localStorage.getItem('vetrx_active_practice_id') || 'prac_mock_malappuram');

    const categories = ['Medicine', 'Procedure Fee', 'Consultation', 'Vaccination', 'Lab Test', 'General Health Care'];

    const notes = options.longStatutory
      ? 'Statutory Healthcare Exemption: All clinical services, surgical operations, medical consultations, diagnostic lab investigations, and in-clinic therapeutic administrations rendered herein are strictly exempt from GST under Ministry of Finance (Dept of Revenue) Notification No. 12/2017-Central Tax (Rate) Serial No. 46 Heading 9993 (Healthcare services by clinical establishment or authorized veterinary medical professional).'
      : (options.notes || 'Thank you for choosing Malappuram Companion Animal Hospital.');
    ...
    const inv = {
      ...
      practitionerId: 1,
      prescriptionId: 1001,
      prescriptionIds: [1001],
      invoiceDate: new Date().toISOString(),
      status: 'issued',
      paymentStatus: docType === 'Payment Receipt' ? 'paid' : 'unpaid',
      notes,
      totalAmount: itemCount * 35000,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    };
    ...
    for (let i = 0; i < itemCount; i++) {
      const itemId = invId * 100 + i + 1;
      const cat = categories[i % categories.length];

      let description = `Clinical Veterinary Service Item #${i + 1}`;
      if (options.longDesc && i === 0) {
        description = 'Comprehensive Bovine Laparoscopic Exploratory Abdominal Examination with Left Flank Incision, Rumenotomy, Debridement and Multi-Layer Suture Repair under Local Infiltration Anesthesia';
      }
      ...
      await putInDb('invoiceItems', invItem);
    }
  ...
  async function testPrescriptionScenario(scenarioId, fileBaseName, medCount, options = {}) {
    ...
    scenarioResults.push({
      scenarioId,
      docType: 'Veterinary Prescription',
      fileBaseName,
      pdfPath: `artifacts/phase5d-pdf-validation/${fileBaseName}.pdf`,
  ...
  async function testBillingScenario(docType, scenarioId, fileBaseName, itemCount, options = {}) {
    ...
    await page.goto(`${TARGET_URL}/invoices/${invId}?type=${encodeURIComponent(docType)}`, { waitUntil: 'networkidle0' });
  ...
  for (const dir of [WORKSPACE_OUT_DIR, BRAIN_OUT_DIR]) {
    fs.writeFileSync(path.join(dir, 'phase5d-validation-summary.json'), JSON.stringify(summary, null, 2));
  }

  console.log('\n======================================================');
  console.log(`PHASE 5D PDF VISUAL VALIDATION COMPLETE`);
